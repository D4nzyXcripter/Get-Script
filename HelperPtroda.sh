...
check_token() {
  ...
  if [ "$USER_TOKEN" = "danzydev" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di Danzpedia${NC}"
    echo -e "${YELLOW}TELEGRAM : @eix1m${NC}"
    echo -e "${YELLOW}WHATSAPP : +6285624297893${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© Danzpedia${NC}"
    exit 1
  fi
  clear
}
...
hackback_panel() {
  ...
  echo -e "                                                       "
  # Minta input dari pengguna
read -p "Masukkan Email Panel: " emailhb
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer Danzpedia   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : Danzpedia      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : Danzpedia  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by Danzpedia  ${NC}"
